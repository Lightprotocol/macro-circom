...
fn parse(contents: String) -> Result<(Vec<Instance>, Vec<CheckUtxo>, String), AnyhowError> {
    let parsed_content = match parser::parser_logic::preprocess(contents.as_str(), 0usize) {
    ...
    let remaining_contents =
        match crate::parsers::circom_parser::CircomCodeParser::new().parse(&contents) {
            ...
            Err(error) => {
                panic!("{}", describe_error(&parsed_content, error));
            }
        };
    let parsing_res = match parser::lang::ParseInstanceParser::new().parse(&parsed_content) {
        Ok(instance) => instance,
        Err(error) => {
    ...
    Ok((vec![instance], checked_utxos, remaining_contents.join("\n")))
}
...
mod tests {
    ...
    fn test_parser() {
        ...
        let parsed_content = match parser::parser_logic::preprocess(content.as_str(), 0usize) {
            Ok(parsed_content) => parsed_content,
            Err(_) => panic!("Preprocess failed"),
        };

        match parser::lang::ParseInstanceParser::new().parse(parsed_content.as_str()) {
            Ok(instance) => instance,
            Err(error) => {
                println!("Parsing check utxo error.");
