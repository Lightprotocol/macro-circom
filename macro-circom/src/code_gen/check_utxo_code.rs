...
impl CodeGenerator for CheckUtxo {
    fn generate_code(&mut self) -> Result<(), MacroCircomError> {
        let template = r#"
...
fn check_for_duplicates(v: &Vec<CheckUtxo>) -> Result<(), MacroCircomError> {
...
for (var i = 0; i < nIns; i++) {
    ...
    checkPoolTypeUtxoName[i].enabled <== isInAppUtxoUtxoName[i] * instruction;
    ...
    checkVerifierAddressUtxoName[i].enabled <== isInAppUtxoUtxoName[i] * instruction;
    ...
    checkTxVersionUtxoName[i].enabled <== isInAppUtxoUtxoName[i] * instruction;
...
"#;
    ...
    fn complete_test() {
        ...
        let parsing_res = match parser::lang::ParseInstanceParser::new().parse(&contents) {
        ...
        let mut checked_utxos = parsing_res.1;
   ...
   ",
        ...
        assert_eq!(
            check_utxo.verifier_address,
            Some((Comparator::Equal, String::from("sthV")))
        );
        assert_eq!(
            check_utxo.tx_version,
            Some((Comparator::Equal, String::from("sthT")))
        );
        assert_eq!(
            check_utxo.pool_type,
            Some((Comparator::Equal, String::from("sthP")))
        );
        assert_eq!(
            check_utxo.utxo_data,
            Some(vec![
        ...
        println!("check_utxo1 code {}", check_utxo1.code);
    }
}
