...
pub fn generate_psp_circom_code(
    ...
    for line in input.lines() {
        ...
        if found_bracket_once {
            if !checked_utxos.is_empty() {
                println!("line: {}", line);
                println!("checked utxo 0 name: {}", checked_utxos[0].name);
                println!(
                    "line match {}",
                    line.starts_with(&format!("{};", checked_utxos[0].name))
                );
            }

            if let Some(index) = checked_utxos
                .iter()
                .position(|utxo| line.starts_with(&format!("{};", utxo.name)))
            {
                remaining_lines.push(checked_utxos[index].code.clone());
                println!("inserted utxo name {}", checked_utxos[index].name);
                checked_utxos.remove(index);
                continue;
            }
        }
        if !found_bracket {
    ...
    if !checked_utxos.is_empty() {
        return Err(MacroCircomError::CheckUtxosNotUsed(
            checked_utxos
                .iter()
                .map(|utxo| utxo.name.clone())
                .collect::<Vec<String>>()
                .join(", "),
        ));
    }
...
mod light_transaction_tests {
    ...
    fn test_parse_light_transaction_functional() {
        ...
        checked_utxo.name = "checkedProgramUtxo".to_string();
